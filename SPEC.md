...
- **M4.2, self-service viewers:** retain Sign in with ChatGPT for identity, add a D1 owner/viewer membership list, and authorize every hosted page and read API server-side. The configured owner can add and revoke viewer emails from the dashboard; unknown signed-in users receive no fleet data. Viewer-management mutations are owner-only, same-origin, and audit logged. Publishing continues through its separate machine token and viewers never gain local supervisor controls.
...
8. A ChatGPT-authenticated account not present in the hosted membership list cannot read a snapshot or enumerate members. The configured owner can add/revoke viewers, viewers can read status only, and the owner cannot be removed.
